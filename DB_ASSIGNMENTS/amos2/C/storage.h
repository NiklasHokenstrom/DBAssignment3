 ...
 * $RCSfile: storage.h,v $
 * $Revision: 1.124 $ $Date: 2014/12/30 13:55:39 $
 * $State: Exp $ $Locker:  $
 *
...
INLINE oidtype hd(oidtype x) 
{ 
  return (listp(x)?fhd(x):interror(ARG_NOT_LIST,x));
}

INLINE oidtype tl(oidtype x) 
{ 
  return (listp(x)?ftl(x):interror(ARG_NOT_LIST,x));
}
                                                /* Head of list */
...
#define longinteger(x) *((LONGINT *)x)
#define getinteger(x) (longinteger(dr(x,integercell)->data))
                                       /* Get C int of INTEGER object handle */
#define getinteger32(x)(int)getinteger(x)
...
EXTERN oidtype new_aligned_object(size_t size, int type);
...
EXTERN oidtype new_symbol(const char *str);                      /* Internal */
EXTERN oidtype new_integer(LONGINT val);                         /* Internal */
EXTERN oidtype new_string(size_t len,const char *str);           /* Internal */
...
#define IntoStringRef(x,into,env) if(stringp(x)) into = getstring(x);\
...
#define StackString(s,p)(p = (char *)alloca(strlen(s)+1), strcpy(p,s), p)
#define IntoString(x,into,env) \
       if(stringp(x)) StackString(getstring(x),into); \
       else if(symbolp(x)) StackString(getpname(x),into); \
       else return lerror(ARG_NOT_STRING,x,env)
                                             /* Safe dereferencing of string.
...
#define IntoInteger32(x,into,env)if(integerp(x)) into = getinteger32(x);\
       else if(realp(x)) into = (int)a_roundi(getreal(x));\
       else return lerror(ARG_NOT_INTEGER,x,env)
                                                   /* Dereference an integer */
...
EXTERN oidtype a_printerror(bindtype env,int errnr,char *msg,oidtype x);
...
EXTERN int a_errornumber(const char *msg);
                                       /* Get the number of an error message */
...
EXTERN int a_puti(LONGINT i, oidtype stream);            /* Write an integer */
...
oidtype a_encodeNumeric(const char *str);

/*** Text streams ***/

EXTERN oidtype new_textstream(size_t len);
EXTERN oidtype a_textstreamread(oidtype stream, size_t from);
                                                        /* Read S-expression */
#define Int32ToString(x, buff) (sprintf(buff,"%d",x),buff)
